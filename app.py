...
# TASARIM BLOĞU (Tırnak işaretleri ve style etiketleri tamdır, yazı olarak görünmez)
st.markdown("""
...
def mail_at(alici, soru, analiz):
    try:
        ...
        <div style="background-color: #050505; padding: 40px; font-family: serif; color: #e0e0e0; border: 2px solid #D4AF37; border-radius: 15px;">
            ...
            <p style="text-align: right; color: #D4AF37; font-size: 18px; font-style: italic;">— Profesör Minerva</p>
        </div>
        """
            ...
            if st.button("KEHANETİ HAZIRLA VE MÜHÜRLE", use_container_width=True):
                with st.spinner("Minerva mühürleri kontrol ediyor..."):
                    try:
                        ...
                        # Model İsmi (models/ eki NotFound hatasını çözer)
                        model = genai.GenerativeModel('models/gemini-1.5-flash')
                        
                        secilen_kartlar = [TAM_DESTE[idx] for idx in st.session_state.sepet]
                        prompt = f"Sert ol. {yas} yaş, {medeni}, {iliski}, {calisma}. Konu: {konu}. Soru: {soru}. Kartlar: {secilen_kartlar}. Analiz yap ve ACI REÇETE ekle."
...
# ADIM 2: ÖDEME
    ...
    st.write("Minerva mektubunu hazırladı. Analizi almak için ona bir kahve ısmarla.")
    
    # Kendi BanaBiKahve linkini tırnak içine yapıştır
    ...
    if st.button("KAHVEYİ ISMARLADIM, GÖNDER"):
        if mail_at(st.session_state.final_email, st.session_state.final_question, st.session_state.final_analysis):
            ...
            st.error("Mail gönderilemedi! Lütfen Secrets ayarlarını (MY_EMAIL_PASSWORD) kontrol et.")
    st.markdown('</div>', unsafe_allow_html=True)
