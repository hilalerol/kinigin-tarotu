...
def mail_at(alici, soru, analiz):
    try:
        ...
        html = f"""
        <div style="background:#000; color:#ddd; padding:20px; border:2px solid #D4AF37; font-family:serif;">
            <h2 style="color:#D4AF37; text-align:center;">MINERVA'NIN KEHANETİ</h2>
            <p><b>Soru:</b> {soru}</p>
            <div style="background:#111; padding:15px; border-radius:10px;">{analiz.replace(chr(10), '<br>')}</div>
            <p style="text-align:right; color:#D4AF37;">— Profesör Minerva</p>
        </div>
        """
...
if st.session_state.step == "form":
    with st.container():
        ...
        email = st.text_input("Analiz mailin:")
        ...
        if len(st.session_state.sepet) == 3 and email and soru:
            if st.button("KEHANETİ HAZIRLA", use_container_width=True):
                with st.spinner("Minerva odaklanıyor..."):
                    try:
                        ...
                        st.rerun()
                    except: st.error("Bir hata oluştu, tekrar dene.")
